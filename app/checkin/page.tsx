...
function CheckInContent() {
  ...
  useEffect(() => {
    if (sessionId) {
      // Find rehearsal by date (sessionId should match date format YYYY-MM-DD)
      const found = rehearsalSchedule.find(r => r.date === sessionId)
      if (found) {
        setRehearsal(found)
      } else {
        setError('Rehearsal session not found')
      }
    } else {
      setError('No rehearsal session ID provided')
    }
  }, [sessionId])
            ...
            {!user ? (
              <div className="space-y-4">
                <p className="text-gray-300 text-center text-sm">
                  Please sign in with Google to check in for this rehearsal.
                </p>
                <button
                  onClick={handleGoogleSignIn}
                  className="w-full bg-white hover:bg-gray-100 text-gray-900 font-semibold py-3 px-6 rounded-lg transition-all duration-200 flex items-center justify-center gap-3"
                >
